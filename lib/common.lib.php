...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="'.$url.'1'.$add.'" class="jump"><i class="icon-angle-double-left"></i>처음</a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="move"><i class="icon-angle-left"></i>이전</a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="'.$url.$k.$add.'">'.$k.'<span class="sound_only">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sr-only">열린</span><strong>'.$k.'</strong><span class="sr-only">페이지</span>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="move"><i class="icon-angle-right"></i>다음</a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="'.$url.$total_page.$add.'" class="jump"><i class="icon-angle-double-right"></i>맨끝</a>'.PHP_EOL;
    }

    if ($str)
        return "<div class=\"pagenation\"><div class='box'>{$str}</div></div>";
    else
        return "";
}
